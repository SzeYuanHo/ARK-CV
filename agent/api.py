...
app = FastAPI(
    ...
    description="AI agent combining vector search and knowledge graph for CV architecture recommendation",
    version="0.1.0",
    lifespan=lifespan
)
    ...
    try:
        ...
        async def generate_stream():
            ...
            try:
                ...
                # Try streaming first, fallback to non-streaming if tools block streaming
                try:
                    logger.debug("Attempting streaming mode with agent.iter()")
                    ...
                    logger.debug("Stream completed, extracting tools")
                    result = run.result
                    tools_used = extract_tool_calls(result)
                    logger.info(f"Streaming mode succeeded. Tools used: {len(tools_used)}")
                    
                except Exception as e:
                    error_msg = str(e)
                    logger.warning(f"Streaming mode error: {error_msg}")
                    
                    if "Tools are not supported in streaming mode" in error_msg or \
                       ("Provider returned error" in error_msg and "Tools" in error_msg):
                        # Fallback to non-streaming when provider doesn't support streaming with tools
                        logger.info("Falling back to non-streaming mode due to tool usage")
                        result = await rag_agent.run(full_prompt, deps=deps)
                        ...
                        logger.info(f"Non-streaming fallback succeeded. Tools used: {len(tools_used)}")
                    else:
                        raise
