...
from .providers import get_embedding_client, get_embedding_model, get_vlm_client, get_vlm_model
...
async def analyze_image_for_cv_features(image_path: str) -> Dict[str, Any]:
    """
    Analyze an image using VLM to extract CV architecture features.
    
    Args:
        image_path: Path to the image file or base64 encoded image
    
    Returns:
        Dictionary with extracted features as structured data
    """
    try:
        import base64
        ...
        # Create prompt for VLM to extract structured features
        prompt = """You are provided with a microscopy image of either material or biological samples. Analyze this image and extract relevant characteristics for computer vision analysis of the sample.

Provide ONLY the following characteristics, one per line, in this exact format:
- Image Quality: [your assessment]
- Contrast Level: [your assessment]
- Illumination Bias: [your assessment]
- Object Geometry: [your assessment]
- Object Distribution: [your assessment]
- Scale Variation: [your assessment]
- Image Type: [your assessment]

You MUST keep each assessment to 4 words maximum. Do NOT add any extra commentary or explanation."""
        ...
        # Extract key points as a list - look for lines starting with "-" or containing ":"
        lines = analysis_text.split('\n')
        for line in lines:
            line = line.strip()
            if line and (line.startswith('-') or ':' in line):
                # Clean up formatting - remove leading bullets/dashes
                clean_line = line.lstrip('-•*').strip()
                if clean_line and len(clean_line) > 3:  # Skip very short lines
                    features["image_characteristics"].append(clean_line)
        
        # If no structured characteristics found, try to extract from full text
        if not features["image_characteristics"]:
            logger.warning("No structured characteristics found, using full analysis text")
            # Split into sentences and use them as characteristics
            sentences = [s.strip() for s in analysis_text.replace('\n', ' ').split('.') if s.strip()]
            features["image_characteristics"] = sentences[:7]  # Limit to 7 characteristics
        
        logger.info(f"Extracted {len(features['image_characteristics'])} characteristics")
